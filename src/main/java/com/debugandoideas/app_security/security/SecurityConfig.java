...
import com.debugandoideas.app_security.services.CustomerUserDetails;
...
public class SecurityConfig {
    @Bean
    @Order(1) //se carga primero pq es necesarion configurar esto antes de los demás permisos etc
    ...
    /*
    ESTE BEAN LO CREO PARA SABER COMO AÑADIR INFOMACION ADICIONAL A NUESTRO TOKEN (CLAIMS O PAYLOAD)
     */
    @Bean
    ...
    private static KeyPair genertateRSA(){
        KeyPair keyPair = null;
        try{
            var keyParGenerator = KeyPairGenerator.getInstance(RSA);
            keyParGenerator.initialize(RSA_SIZE);
            keyPair = keyParGenerator.generateKeyPair();
    ...
    private static RSAKey gereratedKeys(){
        var keyPair = genertateRSA();
