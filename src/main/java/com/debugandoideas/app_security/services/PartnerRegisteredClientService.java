...
import com.debugandoideas.app_security.repositories.PartnerRepository;
...
public class PartnerRegisteredClientService  implements RegisteredClientRepository {
    ...
    public RegisteredClient findByClientId(String clientId) {
        ...
        return partnerOpt.map(partner ->{
                var authorizationGrantTypes = Arrays.stream(partner.getGrantTypes().split(",")).map(AuthorizationGrantType::new).toList();
                var clientAuthorizationMethods = Arrays.stream(partner.getAuthenticationMethods().split(",")).map(ClientAuthenticationMethod::new).toList();
